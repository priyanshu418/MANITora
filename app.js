...
//mongoose.connect("mongodb+srv://admin-akshat:test123@cluster0.puf7o.mongodb.net/manitoraDB?retryWrites=true&w=majority", {useNewUrlParser: true});
  ...
  if(req.isAuthenticated()){
    ...
    User.findById(userId,function(err,user){
      if(currUser._id==userId)
      {
        sameUser=1;
        console.log("GOOD");
        res.render("profile",{isauth:isauth,currUser:currUser,user:user,sameUser:sameUser});
      } else{
        res.render("profile",{isauth:isauth,currUser:currUser,user:user,sameUser:sameUser});
      }
